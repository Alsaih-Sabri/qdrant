...
{
    helpers::time(update_impl(dispatcher, access, collection, request, params)).await
}
    ...
    T: RestRequest + StrictModeVerification,
{
    ...
    let update = request.into_update().await?;
...
pub async fn update_batch(
    ...
    helpers::time(update_batch_impl(
        dispatcher, access, collection, batch, params,
    ))
    .await
}
...
async fn update_batch_impl(
    ...
    for operation in batch.operations {
        let update = operation.into_update().await?;
...
pub trait RestRequest {
    async fn into_update(self) -> Result<Update, StorageError>;
}
...
impl RestRequest for PointInsertOperations {
    async fn into_update(self) -> Result<Update, StorageError> {
        match self {
            PointInsertOperations::PointsBatch(batch) => batch.into_update().await,
            PointInsertOperations::PointsList(list) => list.into_update().await,
        }
    }
}
...
impl RestRequest for PointsBatch {
    async fn into_update(self) -> Result<Update, StorageError> {
        ...
        let batch = convert_batch(batch).await?;
...
impl RestRequest for PointsList {
    async fn into_update(self) -> Result<Update, StorageError> {
        ...
        let points = convert_point_struct(points, InferenceType::Update).await?;
...
impl RestRequest for UpdateVectors {
    async fn into_update(self) -> Result<Update, StorageError> {
        ...
        let points = convert_point_vectors(points, InferenceType::Update).await?;
...
impl RestRequest for UpdateOperation {
    async fn into_update(self) -> Result<Update, StorageError> {
        match self {
            Self::Upsert(upsert) => upsert.into_update().await,
            Self::Delete(delete) => delete.into_update().await,
            Self::UpdateVectors(update_vectors) => update_vectors.into_update().await,
            Self::DeleteVectors(delete_vectors) => delete_vectors.into_update().await,
            Self::SetPayload(set_payload) => set_payload.into_update().await,
            Self::OverwritePayload(overwrite_payload) => overwrite_payload.into_update().await,
            Self::DeletePayload(delete_payload) => delete_payload.into_update().await,
            Self::ClearPayload(clear_payload) => clear_payload.into_update().await,
        }
    }
}
...
macro_rules! batch_request {
    ($batch:ty, $rest:ty, $field:ident) => {
        impl RestRequest for $batch {
            async fn into_update(self) -> Result<Update, StorageError> {
                <$rest>::from(self).into_update().await
            }
        }
